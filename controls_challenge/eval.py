...
if __name__ == "__main__":
  ...
  for controller_cat, controller_type in [('baseline', args.baseline_controller), ('test', args.test_controller)]:
    ...
    for data_file in tqdm(files[SAMPLE_ROLLOUTS:], desc=f"{controller_cat} rollouts"):
      result = run_rollout(data_file, controller_type, args.model_path, debug=False)
      costs.append({'controller': controller_cat, **result[0]})
